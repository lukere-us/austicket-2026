...
const DEFAULTS = {
  ...
  scrimSideOpacity: 55,
  counterTicketsBase: 0,
  counterTicketsLabel: 'Ticket Booked with us',
  counterAusBase: 0,
  counterAusLabel: 'Listed event in AUS',
  counterNzBase: 0,
  counterNzLabel: 'Listed event In NZ',
  counterCustomersBase: 0,
  counterCustomersLabel: 'Happy Customers',
  counterAnimationMs: 2000,
}
...
export const FIELD_GROUPS = [
  ...
  {
    id: 'counters',
    label: 'Banner stat counters',
    fields: [
      {
        key: 'counterTicketsBase',
        label: '01 — Tickets booked (base value)',
        type: 'number',
        min: 0,
        max: 10000000,
        step: 1,
        help: 'Shown total = this value + booking click analytics count.',
      },
      { key: 'counterTicketsLabel', label: '01 — Label', type: 'text' },
      {
        key: 'counterAusBase',
        label: '02 — Listed events in AUS (base value)',
        type: 'number',
        min: 0,
        max: 10000000,
        step: 1,
        help: 'Shown total = this value + published listings with shows in Australia.',
      },
      { key: 'counterAusLabel', label: '02 — Label', type: 'text' },
      {
        key: 'counterNzBase',
        label: '03 — Listed events in NZ (base value)',
        type: 'number',
        min: 0,
        max: 10000000,
        step: 1,
        help: 'Shown total = this value + published listings with shows in New Zealand.',
      },
      { key: 'counterNzLabel', label: '03 — Label', type: 'text' },
      {
        key: 'counterCustomersBase',
        label: '04 — Happy customers (base value)',
        type: 'number',
        min: 0,
        max: 10000000,
        step: 1,
        help: 'Shown total = this value + registered user count.',
      },
      { key: 'counterCustomersLabel', label: '04 — Label', type: 'text' },
      {
        key: 'counterAnimationMs',
        label: 'Counter animation duration (ms)',
        type: 'number',
        min: 500,
        max: 8000,
        step: 100,
        help: 'How long each counter animates from 0 to its total on the home page.',
      },
    ],
  },
  {
